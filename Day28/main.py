from itertools import count
from tkinter import Tk, Canvas, PhotoImage, Button, Label
from math import floor
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None
# ---------------------------- TIMER RESET ------------------------------- # 
def reset_timer():
    window.after_cancel(timer)
    canvas.itemconfig(timer_text, text="00:00")
    timer_label.config(text="Timer", fg=GREEN)
    check_label.config(text="")
    global reps
    reps = 0

# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start_timer():
    global reps
    reps += 1
    work_sec = int(WORK_MIN * 60)
    short_break_sec = int(SHORT_BREAK_MIN * 60)
    long_break_sec = int(LONG_BREAK_MIN * 60)

    if reps%8 == 0:
        countdown = long_break_sec
        timer_label.config(text="Break", fg=RED)
    elif reps%2 == 0:
        countdown = short_break_sec
        timer_label.config(text="Break", fg=PINK)
    else:
        countdown = work_sec
        timer_label.config(text="Work", fg=GREEN)

    count_down(countdown)

# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(countdown):
    #for updating any item inside the canvas, we need to first
    #use the canvas inside which we have that item and then update the value
    count_min = floor(countdown/60)
    count_sec = countdown % 60
    if count_sec < 10:
        count_sec=f"0{count_sec}"
    canvas.itemconfig(timer_text, text=f"{count_min}:{count_sec}")

    if countdown > 0:
        global timer
        timer = window.after(1000, count_down, countdown -1)
    else:
        start_timer()
        if reps%2 == 0:
            mark_present = check_label["text"]+"✔️"
            check_label.config(text=mark_present,bg=YELLOW, fg=GREEN)
        # mark = ""
        # work_sessions = floor(reps/2)
        # for _ in range(floor())

# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=YELLOW)

# def say_something(thing):
#     print(thing)

# window.after(2000, say_something, "Hello") # after method can be used to execute a function after a given time in ms

#Start button
start_btn = Button()
start_btn.config(text="Start", highlightthickness=0, command=start_timer)
start_btn.grid(column=0, row=2)

#Timer Test
timer_label = Label()
timer_label.config(text="Timer", fg=GREEN, font=(FONT_NAME, 30, "bold"), bg=YELLOW)
timer_label.grid(column=1, row=0)

#Adding image to the window
canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
tomato_photo = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=tomato_photo)
timer_text = canvas.create_text(103, 130, text="00:00", fill="white", font=(FONT_NAME, 35, "bold"))
canvas.grid(column=1, row=1)

#Check mark
check_label = Label()
check_label.config(bg=YELLOW, fg=GREEN)
check_label.grid(column=1, row=3)

#Reset button
reset_btn = Button()
reset_btn.config(text="Reset", highlightthickness=0, command=reset_timer)
reset_btn.grid(column=2, row=2)





window.mainloop()
